...
- **Premium（サブスクリプション）**:
  ...
  - アプリは常に最新の購読状態を反映し、**アプリ外部での購読解除後も、復帰時には正しく無料ユーザーとして動作する**こと。
...
- **モジュール**: `:app`（UI/Android 依存）, `:core`（共通基盤）, `:data`（Repository）, `:domain`（UseCase/モデル）, `:quiz-feature-history`（クイズ履歴機能）
...
### 3.7 依存関係の変更に関する厳格なルール
過去の失敗に基づき、依存関係の追加・更新・削除は、プロジェクト全体に予期せぬ影響を及ぼすリスクが極めて高い行為と定義する。したがって、`build.gradle.kts`ファイル（モジュールレベル、プロジェクトレベル問わず）を編集する際は、以下のプロセスを**絶対に遵守する**こと。

1.  **【必須】変更目的の明確化**:
    *   なぜそのライブラリの追加・更新・削除が必要なのか、目的を明確に記述する。

2.  **【必須】`gradle sync`の個別実行**:
    *   `build.gradle.kts`への変更は、**一度に1つの依存関係のみ**とする。
    *   変更を加えた直後、**必ず`gradle_sync()`ツールを実行**し、IDEとGradleの状態を同期させる。

3.  **【必須】ビルド検証**:
    *   `gradle sync`が成功した後、**必ず`gradle_build("app:assembleDebug")`を実行**し、変更が他のモジュールやビルドプロセス全体を破壊していないことを確認する。

4.  **【必須】段階的変更の徹底**:
    *   複数の依存関係を変更する必要がある場合でも、**「1変更 → 1Sync → 1ビルド」のサイクルを厳格に守る**こと。これにより、問題が発生した際に原因の特定が容易になる。

ビルドスクリプトの安易な変更は、最も修正コストの高い失敗に繋がることを常に意識すること。
