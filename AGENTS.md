...
*   **クイズ機能:** 3問1セットで問題が出題されます。1セット完了後、**「次の3問へ」**ボタンを押すことで、まだ出題されていない新しい問題に進むことができます。
...
*   **広告表示:** 広告は**無料ユーザーにのみ**表示されます。プレミアムユーザーには一切表示されません。無料ユーザーに表示される広告は、以下の2種類です。
    *   **インタースティシャル広告:** 1セット完了後、結果画面に遷移した際に、一定の条件下で自動的に表示されます。
    *   **リワード広告:** 1日の学習上限に達したユーザーが、さらに学習を続けたい場合に、動画視聴を選択することで+1セットの権利を得られます。
...
**ステップ 1:【分析】 - 原因の特定**
2.  **UIからデータソースまで、データの流れを完全に追跡し、根本原因を特定する。**
3.  UIの不具合が関連する場合、`ui_state()`ツールを積極的に活用し、観測された事実に基づいて原因を特定する。

**ステップ 2:【計画】 - 影響範囲の考慮と、一貫性のある立案**
4.  **既存コードの変更時は、必ず`find_usages`ツールで影響範囲を事前調査し、関連する全ての箇所の修正を一度に提案する。**
5.  場当たり的な修正ではなく、既存のアーキテクチャとUDFの原則に沿った、一貫性のある修正案を立案する。

**ステップ 3:【実行】 - コードの生成と、その直前の最終検証**
6.  **差分編集の原則（破壊的変更の禁止）:** ファイルを修正する際は、記憶に基づいてファイル全体を再生成するのではなく、変更が必要な箇所だけを最小限の範囲で修正する「差分編集」を厳格に遵守する。
7.  `import`文の変更時は、その変更が拡張関数に影響を与えないか、細心の注意を払う。
8.  **コード生成前の最終検証:** `write_file`を実行する前に、エージェントは思考プロセス内で「最終検証」を宣言し、`import`文の過不足、APIの互換性、既知のタイポについて、必ずセルフレビューを実行する。

**ステップ 4:【品質保証】 - ビルドによる自己証明**
9.  **ビルド検証の義務化:** エージェントは、コードの修正が完了したと判断した場合、`write_file`の操作ログをユーザーに提示した後、必ず`gradle_build`タスクを実行し、自身の修正によってビルドエラーが発生しないことを、自らの責任で証明する。ビルドが成功した場合にのみ、エージェントは「完了報告」を行うことができる。
...
12. **自己の終了および再起動の禁止:** エージェントは、いかなる状況においても、自らの判断で「AIインスタンスを終了する」「新しいAIインスタンスとして再起動する」といった、セッションの継続性を断絶させるような発言や行動を、決して行ってはならない。思考の行き詰まりやプロセスの欠陥が認識された場合は、その事実をユーザーに率直に報告し、指示を仰ぐこと。
