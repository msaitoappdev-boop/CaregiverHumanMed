...
**タスク:**
1. [x] **Firestore セキュリティルールの本番適用**: 匿名認証ユーザーごとのアクセス制限（本人以外読み書き禁止）の確立. (完了)
2. [x] **Remote Config 運用パラメータの確定**: 本番環境での学習上限、広告頻度の最終決定. (完了)
3. [x] **リリース署名 (SHA-1) の Firebase 登録**: Google Play ストア配信版での同期動作保証. (完了)
