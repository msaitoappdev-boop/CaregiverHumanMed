...
2. [x] **ドメイン層の強化 (`:quiz-core-domain`)**: `GetWeaknessQuestionsUseCase`（フォールバック対応）および `GetLearningAnalysisUseCase`（日・週・月別集計）の実装。 (完了)
3. [x] **学習分析モジュールの実装 (`:quiz-feature-analysis`)**: 進捗グラフ、分野別正解率リスト（日本語化対応）、日別トレンド（%表示付き）の視覚化。 (完了)
4. [x] **弱点特訓フローの洗練**: モード管理を SettingsProvider へ統合し、セットを跨いだ持ち回り出題を実現。 (完了)
...
- **学習分析のさらなる高度化（Premium価値向上）**:
    - **合格へのレーダーチャート**: 全カテゴリの習熟度を一目で把握し、バランスの良い学習を促す。
    - **試験突破推定スコア**: 現在の実力から本試験での到達予想スコアを算出し提示。
    - **学習継続カレンダー（ヒートマップ）**: 学習実績をカレンダー形式で視覚化し、習慣化を支援。
    - **分野別・即時弱点克服**: 分析画面から正解率の低いカテゴリを直接指定して特訓を開始できる動線。
