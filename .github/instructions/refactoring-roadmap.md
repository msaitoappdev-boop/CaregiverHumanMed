...
- **Spokes（スポーク: クイズ共通機能群）**: 全てのコア機能を含む、共有ライブラリモジュールのセット。汎用的なものは接頭辞からドメイン名を外します。
    ...
    - `:quiz-feature-analysis` (新規追加)
    - `:quiz-feature-weakness` (新規追加)
...
**目標:** 3月末の初回リリースに向け、課金する価値のある「弱点特訓モード」と「成長の見える化（グラフ分析）」を実装し、収益化の基盤を強化する。

**タスク:**
1. **データ層の拡張 (`:quiz-core-data`)**:
    - [ ] `WrongAnswerRepository` の実装。問題ごとの正解/不正解回数、最終解答日時を DataStore で永続化。
    - [ ] クイズ結果画面からの統計データ自動更新ロジックの追加。
2. **ドメイン層の強化 (`:quiz-core-domain`)**:
    - [ ] `GetWeaknessQuestionsUseCase`: 誤答率の高い問題を抽出。
    - [ ] `GetLearningAnalysisUseCase`: カテゴリ別正解率、日別スコア推移（直近30日）の算出。
3. **学習分析モジュールの実装 (`:quiz-feature-analysis`)**:
    - [ ] 分析レポート画面の実装（スコア推移グラフ、カテゴリ別習熟度チャート）。
    - [ ] 成長の実感を与えるサマリテキストの動的表示。
4. **弱点特訓モジュールの実装 (`:quiz-feature-weakness`)**:
    - [ ] 苦手問題のみを 3 問セットで出題する「弱点克服モード」フローの実装。
    - [ ] ホーム画面への導線追加（Premium 限定ロック解除ロジック）。
