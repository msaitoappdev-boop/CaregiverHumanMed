...
3. [x] **テーマ・カラーのセマンティック化**: QuizColors を導入し、正解・不正解の色指定を Hub 層で一元管理可能に。 (完了)
...
### フェーズ4：パッケージ名変更に伴う外部サービス再設定（進行中）
...
3. [ ] **課金（Google Play Billing）の最終テスト**:
    - [x] Play Console で定期購入アイテムを作成済み。
    - [ ] **【未完了】テストユーザーによる実機での購入フロー・復元フローの最終確認**。
4. [ ] **既存ユーザーの移行戦略**:
    - [ ] 旧アプリからのアップデート通知、またはストア説明文での誘導案を策定。
