...
3. [ ] **Route/Screen の完全分離**: 各`...Route.kt`ファイルがUI定義（Screen）とロジック（Route）を両方含んでいる現状を解消する。UI表示ロジックを、対応する`...Screen.kt`ファイルへ**移管**し、`...Route.kt`をロジック（状態管理とイベント伝達）に専念させることで、責務を完全に分離する。
