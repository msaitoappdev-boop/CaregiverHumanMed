...
### フェーズ 2.8：最新 OS (Android 14/15) 適合とコンテンツ最終点検（進行中）

**目標:** リリース品質を担保するため、最新 OS での挙動確認とコンテンツの整合性チェックを行う。

**タスク:**
1. [x] **最新通知権限への対応**: Android 13 以降の `POST_NOTIFICATIONS` 権限要求フローを実装済み。 (完了)
2. [ ] **バックグラウンド実行の安定性確認**: Android 14 以降の省電力規制下で `WorkManager` が期待通り実行されるかの検証。
3. [ ] **Edge-to-Edge 表示の検証**: Android 15 のシステム全画面表示設定において、UI 要素がシステムバーと重ならないかの確認。
