...
### フェーズ2.5：ナビゲーション定義の分離とアーキテクチャの一貫性向上

**目標:** `:app`モジュールに起因する最後の依存関係違反を解消し、ComposeのアーキテクチャをAGENTS.mdの規約に完全に準拠させる。

**タスク:**
1. [ ] **ナビゲーション定義の分離 (`NavRoutes` → `...Destinations`)**: 肥大化した`NavRoutes`オブジェクトを廃止し、各機能に対応するルート文字列定義を、`:quiz-core-navigation`モジュール内に`...Destinations`オブジェクトとして個別に作成・移行する。
2. [ ] **Route/Screen の完全分離**: 各`...Route.kt`ファイルがUI定義（Screen）とロジック（Route）を両方含んでいる現状を解消する。UI表示ロジックを、対応する`...Screen.kt`ファイルへ**移管**し、`...Route.kt`をロジック（状態管理とイベント伝達）に専念させることで、責務を完全に分離する。
