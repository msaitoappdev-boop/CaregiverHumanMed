...
### フェーズ 2.7：スポークモジュールの独立性強化（量産化準備）

**目標:** 各機能モジュール（Spoke）が特定のアプリケーション（Hub）に依存せず、完全に再利用可能な状態にする。

**タスク:**
1. [x] **リソース参照の完全疎結合化**: 調査の結果、現状で Spoke から Hub への不適切な依存がないことを確認。 (完了)
2. [x] **問題データ取得の DI 化**: `QuizAssetConfig` による動的なロードパス指定を実装。 (完了)
3. [ ] **テーマ・カラーのセマンティック化**: Spoke モジュール内の色指定をすべて `MaterialTheme.colorScheme` 経由に統一し、ハードコードを排除。
4. [ ] **Firebase/Remote Config 定数の共通化**: モジュールを跨いで使用するキー名を `:quiz-core-common` 等で一元管理し、量産時の設定ミスを防止。
